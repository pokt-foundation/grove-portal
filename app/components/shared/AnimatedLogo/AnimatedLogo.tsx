...
export default function AnimatedLogo({ done = false }: { done?: boolean }) {
  return (
    <svg width={100} height={100} viewBox="0 0 100 100" overflow="visible">
      <Outline />
      <Outline gradient done={done} />
      <Outline done={done} />
      <Outline gradient end done={done} />
      <defs>
        <linearGradient
          id={GRADIENT_ID}
          x1={0}
...
function Outline({ gradient = false, done = false, end = false, ...props }) {
  return (
    <path
      className={clsx({
